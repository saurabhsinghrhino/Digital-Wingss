...
export default function HomeImageCarousel({ images = [] }) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isPaused, setIsPaused] = useState(false);

  // Normalize image data format (supporting string URLs or objects)
  ...
  // Next / Previous slide actions with infinite wrapping
  ...
  // Autoplay Timer (Every 3 seconds)
  useEffect(() => {
  ...
  // Touch gesture state (Mobile swipe)
  ...
  // Mouse drag state (Desktop mouse swipe)
  ...
  return (
    <div
      ...
      aria-label="DigitalWings Showcase Image Carousel"
    ...
    >
      ...
      >
        {/* Staged slide layers for smooth crossfades */}
        {slides.map((slide, idx) => {
          const isActive = idx === currentIndex;
          return (
            <div
              key={idx}
              className={`absolute inset-0 w-full h-full transition-all duration-500 ease-out motion-reduce:transition-none ${
                isActive
        ...
        {/* Subtle signature ice-blue bottom accent line */}
